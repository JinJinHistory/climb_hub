...
export const resolvers = {
  Query: {
    ...
    routeUpdates: async (_: any, { gymId, type, limit = 10, offset = 0 }: {
      gymId?: string;
      type?: string;
      limit?: number;
      offset?: number;
      ...
      query += ' ORDER BY ru.update_date DESC LIMIT $' + paramIndex + ' OFFSET $' + (paramIndex + 1);
  ...
  Mutation: {
    ...
    updateBrand: async (_: any, { id, input }: { id: string; input: any }) => {
      ...
      values.push(id);
      const result = await pool.query(
        `UPDATE brands SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${paramIndex} RETURNING *`,
        values
      );
      return result.rows[0];
    },
    ...
    createGym: async (_: any, { input }: { input: any }) => {
      const result = await pool.query(`
        ...
        RETURNING *
      `, [
        input.brandId, input.name, input.branchName, input.instagramUrl, input.instagramHandle,
        input.address, input.phone, input.latitude, input.longitude, input.isActive
      ]);
      
      const gym = result.rows[0];
      const brandResult = await pool.query('SELECT * FROM brands WHERE id = $1', [gym.brand_id]);
    ...
    updateGym: async (_: any, { id, input }: { id: string; input: any }) => {
      ...
      values.push(id);
      const result = await pool.query(
        `UPDATE gyms SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${paramIndex} RETURNING *`,
        values
      );
    ...
    createRouteUpdate: async (_: any, { input }: { input: any }) => {
      const result = await pool.query(`
        ...
        RETURNING *
      `, [
        input.gymId, input.type, input.updateDate, input.title, input.description,
        input.instagramPostUrl, input.instagramPostId, input.imageUrls, input.rawCaption, input.parsedData, input.isVerified
      ]);
    ...
    updateRouteUpdate: async (_: any, { id, input }: { id: string; input: any }) => {
      const fields = [];
      const values = [];
      let paramIndex = 1;
      ...
      values.push(id);
      const result = await pool.query(
        `UPDATE route_updates SET ${fields.join(', ')}, updated_at = NOW() WHERE id = $${paramIndex} RETURNING *`,
        values
      );
    ...
    createCrawlLog: async (_: any, { input }: { input: any }) => {
      const result = await pool.query(`
        ...
        RETURNING *
      `, [
        input.gymId, input.status, input.postsFound, input.postsNew,
        input.errorMessage, input.startedAt, input.completedAt
      ]);
    ...
    updateCrawlLog: async (_: any, { id, input }: { id: string; input: any }) => {
      const fields = [];
      const values = [];
      let paramIndex = 1;
      ...
      values.push(id);
      const result = await pool.query(
        `UPDATE crawl_logs SET ${fields.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
        values
      );
